

--  category table

CREATE TABLE MARKETCATEGORY
(
  CAT_ID   NUMBER       NOT NULL
    PRIMARY KEY,
  CAT_NAME VARCHAR2(30) NOT NULL,
  EMAIL    VARCHAR2(30) NOT NULL
    CONSTRAINT M_CATEGORY_M_EMAIL_FK
    REFERENCES MARKETUSER
);

--  product table
CREATE TABLE MARKETPRODUCT
(
  PRODUCT_NAME       VARCHAR2(30) NOT NULL
    PRIMARY KEY,
  PRODUCT_PRICE      NUMBER       NOT NULL,
  PRODUCTION_DATE    DATE         NOT NULL,
  EXPIRED_DATE       DATE         NOT NULL,
  PRODUCTION_COMPANY VARCHAR2(30) NOT NULL,
  ADMIN_EMAIL        VARCHAR2(30) NOT NULL
    CONSTRAINT M_PRODUCT_M_USER_FK
    REFERENCES MARKETUSER,
  PRODUCT_QUANTITY   NUMBER
);

--  for all user in the system ----> seller or admin
-- auto-generated definition
CREATE TABLE MARKETUSER
(
  FNAME      VARCHAR2(30) NOT NULL,
  LNAME      VARCHAR2(30),
  PASSWORD   VARCHAR2(60) DEFAULT NULL,
  CREATED_AT TIMESTAMP(6),
  EMAIL      VARCHAR2(30) NOT NULL
    PRIMARY KEY,
  ROLE       VARCHAR2(30) NOT NULL
);

-- for transactions Transaction_Total and Transaction_Profit should be auto calculated
CREATE TABLE MARKETTRANSACTIONS(
  Transaction_id  NUMBER NOT NULL PRIMARY KEY AUTO_INCREMENT,
T_USER_EMAIL VARCHAR2(30) NOT NULL REFERENCES MARKETUSER,
T_PRODUCT_NAME VARCHAR(30) NOT NULL REFERENCES MARKETPRODUCT,

TRANSACTION_DATE DATE,
SELLING_PRICE NUMBER NOT NULL,
QUANTITY NUMBER NOT NULL,

TRANSACTION_TOTAL NUMBER NOT NULL,

TRANSACTION_PROFIT NUMBER NOT NULL,

);



